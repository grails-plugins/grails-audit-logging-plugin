...
=== 6.x to 7.x Checklist

. Upgrade the application to Grails 8 and Java 21.
. Replace the 6.x runtime dependency with `org.grails.plugins:audit-logging:7.0.0-SNAPSHOT` while the 7.x snapshot is the available development artifact.
. Add `https://central.sonatype.com/repository/maven-snapshots/` to the build repositories when resolving the snapshot. Use Maven Central and a released version when a released 7.x artifact is available.
. Keep the existing `AuditLogEvent` class, its mappings and constraints, and all `grails.plugin.auditLog.*` configuration.
. Do not rerun `audit-quickstart`. It does not overwrite the existing class and can append duplicate configuration. If you intentionally redesign the audit domain, update the class and configuration manually.
. Run `./gradlew --refresh-dependencies shell` if the CLI companion is not discovered.

=== Breaking Changes

Plugin 7.x supports Grails 8 and Java 21 only. It does not support Grails 7.

The CLI companion is now a separate artifact. Consumer applications still declare only `org.grails.plugins:audit-logging`. The runtime JAR advertises `org.grails.plugins:audit-logging-cli` through `Grails-Cli-Artifact`, and Grails 8 provisions it only when executing CLI commands. Do not add the companion directly.

`audit-quickstart` remains available as `grails audit-quickstart <package> <DomainClass>`, or from `./gradlew shell` followed by `audit-quickstart <package> <DomainClass>`. It is a legacy script in `META-INF/commands`, not an `ApplicationCommand`, so do not use `runCommand` or a generated `auditQuickstart` task.

=== Unchanged Behavior

The audit domain class remains application-owned. Existing customizations continue to be the source of truth, and configuration keys remain under `grails.plugin.auditLog.*`.

The auditing and stamping traits, including `Auditable` and `Stampable`, keep their established usage. Follow the configuration guide for supported settings.
