...
[WARNING]
====
Since version 2.0.0, the configuration key has changed from `auditLog` to `grails.plugin.auditLog`.
...
|*(Required)* Use `grails audit-quickstart <package> <DomainClass>` to create the artifact in a new application. Existing applications should retain their configured audit domain class.
